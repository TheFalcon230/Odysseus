...
namespace Odysseus
{
	...
	void OpenGLShader::CompileOrGetVulkanBinaries(const std::unordered_map<GLenum, std::string>& shaderSources)
	{
		...
		for (auto&& [stage, source] : shaderSources)
		{
			...
			else
			{
				...
				{
					ODC_CORE_ERROR(module.GetErrorMessage());
					ODC_CORE_ASSERT(false);
				}
	...
	void OpenGLShader::CreateProgram()
	{
		...
		if (isLinked == GL_FALSE)
		{
			...
			ODC_CORE_ERROR("Shader linking failed ({0}):\n{1}", m_FilePath, infoLog.data());
	...
	void OpenGLShader::Reflect(GLenum stage, const std::vector<uint32_t>& shaderData)
	{
		...
		ODC_CORE_TRACE("OpenGLShader::Reflect - {0} {1}", Utils::GLShaderStageToString(stage), m_FilePath);
		ODC_CORE_TRACE("    {0} uniform buffers", resources.uniform_buffers.size());
		ODC_CORE_TRACE("    {0} resources", resources.sampled_images.size());

		ODC_CORE_TRACE("Uniform buffers:");
		for (const auto& resource : resources.uniform_buffers)
		{
			...
			ODC_CORE_TRACE("  {0}", resource.name);
			ODC_CORE_TRACE("    Size = {0}", bufferSize);
			ODC_CORE_TRACE("    Binding = {0}", binding);
			ODC_CORE_TRACE("    Members = {0}", memberCount);
		}
	}
}